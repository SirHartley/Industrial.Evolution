...
import static com.fs.starfarer.api.splinterFleet.plugins.FleetUtils.USE_ABILITY_MEM_KEY;
...
public abstract class BaseSplinterFleetAssignmentAIV2 extends BaseAssignmentAI implements SplinterFleetAssignmentAIV2, CampaignEventListener, GateTransitListener {
    ...
    protected void giveInitialAssignments() {
        log.info("SplinterFleet AI Init");
    ...
    public void checkForJumpActions() {
        ...
        if (!jumpInfoMap.isEmpty() && fleet.getAI() != null && !fleet.getTags().contains(CURRENTLY_JUMPING_FLAG)) {
            ...
            if (fleet.getContainingLocation().getId().equals(jumpInfo.to.getDestination().getContainingLocation().getId())) {
                ...
                log.info("removing jump order to " + jumpInfo.to.getDestination().getContainingLocation().getName() + ", already in location");
                return;
            }

            log.info("Initializing jump for " + fleet.getName() + ": from " + (jumpInfo.from != null ? jumpInfo.from.getContainingLocation().getName() : "null") + " to " + jumpInfo.to.getDestination().getContainingLocation().getName());
            log.info("there are " + (jumpInfoMap.size() - 1) + " jumps remaining in the list");
    ...
    private void doOverrideActions() {
        ...
        if (isOverridden != this.isOverridden) {
            log.info("SplinterFleet AI performing override actions for " + Behaviour.getFleetBehaviour(fleet, false));
            ...
            if (!isOverridden) {
                ...
                switch (Behaviour.getFleetBehaviour(fleet, false)) {
                    case RETURN_TO_PLAYER_AND_MERGE:
                        log.info("SplinterFleet return to player mode");
                        ...
                        break;
                    case DORMANT:
                        log.info("SplinterFleet Dormant mode");
                        ...
                        break;
                    case COMBAT:
                        log.info("SplinterFleet combat mode");
                    ...
                    case HEAD_TO_PLAYER_ON_TARGET:
                        log.info("setting head to player assignment");
    ...
    public void useAbility(String abilitiy, boolean state) {
        ...
        if (state && ability != null && ability.isUsable() && !ability.isActive())
            fleet.getMemoryWithoutUpdate().set(USE_ABILITY_MEM_KEY, abilitiy);
        else if (!state && ability != null && ability.isActive())
            fleet.getMemoryWithoutUpdate().unset(USE_ABILITY_MEM_KEY);
    }
    ...
    public void addToJumpList(SectorEntityToken from, JumpPointAPI.JumpDestination to) {
        log.info("Adding target to jump list for " + fleet.getName() + ": from " + (from != null ? from.getContainingLocation().getName() : "null") + " to " + to.getDestination().getContainingLocation().getName());
