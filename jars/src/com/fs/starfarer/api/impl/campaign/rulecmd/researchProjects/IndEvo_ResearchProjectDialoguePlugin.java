...
public class IndEvo_ResearchProjectDialoguePlugin extends BaseCommandPlugin implements InteractionDialogPlugin {
    ...
    public static final Logger log = Global.getLogger(IndEvo_ResearchProjectDialoguePlugin.class);

    @Override
    public boolean execute(String ruleId, InteractionDialogAPI dialog, List<Misc.Token> params, Map<String, MemoryAPI> memoryMap) {
        if (!(dialog instanceof com.fs.starfarer.api.impl.campaign.rulecmd.salvageYardsRules.IndEvo_InitSYCustomProductionDiag)) {
            this.dialog = dialog;
    ...
    public void refreshCustomPanel() {
        new IndEvo_ResearchProjectSidePanelCreator().showPanel(dialog);
    }
    ...
    public static IndEvo_ResearchProjectDialoguePlugin getCurrentDialoguePlugin() {
        return (IndEvo_ResearchProjectDialoguePlugin) Global.getSector().getMemoryWithoutUpdate().get(DIALOGUE_PLUGIN_TEMP_INSTANCE_KEY);
    }
    ...
    private void addTooltip(TextPanelAPI panel) {
        ...
        int numOpen = IndEvo_ResearchProjectTemplateRepo.getNumProjectsPlayerCanContribute();
        if (numOpen > 0)
        ...
        if (rewardProjId != null && inputProjId == null) {
            IndEvo_ResearchProject project = IndEvo_ResearchProjectTemplateRepo.RESEARCH_PROJECTS.get(rewardProjId);
            ...
            IndEvo_ResearchProject project = IndEvo_ResearchProjectTemplateRepo.RESEARCH_PROJECTS.get(inputProjId);
            if (project != null) {
                ...
                for (IndEvo_RequiredItem item : project.getRequiredItems()) {
                    String progress = IndEvo_StringHelper.getAbsPercentString(item.points / requiredPoints, false);
