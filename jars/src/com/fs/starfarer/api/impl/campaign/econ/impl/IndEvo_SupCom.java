...
import com.fs.starfarer.api.RandomCollection;
...
import static com.fs.starfarer.api.impl.campaign.econ.impl.installableItemPlugins.IndEvo_SpecialItemEffectsRepo.SIMULATOR_BASE_INCREASE;

public class IndEvo_SupCom extends IndEvo_SharedSubmarketUser implements EconomyTickListener, IndEvo_newDayListener {
    ...
    private static void debugMessage(String Text) {
        ...
        if (DEBUG) {
            Global.getLogger(IndEvo_SupCom.class).info(Text);
        }
    }
    ...
    private void getNextCore() {
        if (getAICoreId() == null && market.hasSubmarket(IndEvo_ids.SHAREDSTORAGE) && market.isPlayerOwned()) {
            SubmarketAPI storage = market.getSubmarket(IndEvo_ids.SHAREDSTORAGE);
    ...
    public void unapply() {
        ...
        List<MarketAPI> PlayerMarketsInSystem = IndEvo_IndustryHelper.getMarketsInLocation(this.market.getStarSystem(), this.market.getFactionId());
        for (MarketAPI PlayerMarket : PlayerMarketsInSystem) {
            PlayerMarket.getIncomeMult().unmodify(IndEvo_ids.SUPCOM);
        }
    ...
    private void modifyAllMarketIncome() {
        List<MarketAPI> playerMarketsInSystem = IndEvo_IndustryHelper.getMarketsInLocation(market.getStarSystem(), market.getFactionId());
        ...
        for (MarketAPI playerMarket : playerMarketsInSystem) {
            if (playerMarket.hasIndustry(getId())) {
                IndEvo_SupCom supCom = (IndEvo_SupCom) playerMarket.getIndustry(IndEvo_ids.SUPCOM);
                float currentIncrease = supCom.getCurrentIncrease();
    ...
    private void coreDump() {
        if (!market.hasIndustry(IndEvo_ids.LAB)) return;
    ...
    private String aiCoreType() {
        if (Commodities.ALPHA_CORE.equals(this.lastApplied)) {
            RandomCollection<String> AiCoreOut = new RandomCollection<>();
            AiCoreOut.add(40, Commodities.GAMMA_CORE);
            AiCoreOut.add(35, Commodities.BETA_CORE);
            AiCoreOut.add(25, Commodities.ALPHA_CORE);

            return AiCoreOut.next();

        } else {
            RandomCollection<String> AiCoreOut = new RandomCollection<>();
            AiCoreOut.add(65, Commodities.GAMMA_CORE);
            AiCoreOut.add(25, Commodities.BETA_CORE);
            AiCoreOut.add(10, Commodities.ALPHA_CORE);
    ...
    public List<SpecialItemData> getVisibleInstalledItems() {
        ...
        if (lastApplied != null) l.add(IndEvo_Items.convertAICoreToSpecial(lastApplied));
    ...
    protected void addRightAfterDescriptionSection(TooltipMakerAPI tooltip, IndustryTooltipMode mode) {
        if (!this.isBuilding() && this.isFunctional()) {
            ...
            if (market.isPlayerOwned() || currTooltipMode == IndustryTooltipMode.NORMAL) {
                ...
                if (this.lastApplied == null && currTooltipMode == IndustryTooltipMode.ADD_INDUSTRY) {
                    ...
                    tooltip.addPara("The current bonus for this colony is: %s", 10f, Misc.getHighlightColor(), IndEvo_StringHelper.getAbsPercentString(market.getIncomeMult().getMultStatMod(getModId()).getValue() - 1, false));
                }
            }
        }
    }
...
//AI core stuff
    ...
    public Float getApplicableSimulatorBonus() {
        ...
        for (MarketAPI market : Misc.getFactionMarkets("player")) {
            ...
            if (market.hasIndustry(IndEvo_ids.SUPCOM)) {
                IndEvo_SupCom supCom = (IndEvo_SupCom) market.getIndustry(IndEvo_ids.SUPCOM);
                if (supCom.isFunctional() && supCom.getLastApplied() != null && supCom.getSpecialItem() != null) {
                    float dist = Misc.getDistanceLY(this.market.getLocationInHyperspace(), supCom.market.getLocationInHyperspace());
                    if (dist < minDist) {
                        minDist = dist;
                        nearest = market;
                        ...
                        if (dist > RANGE_LY_TEN) bonus = 0f;
                            ...
                            bonus = supCom.getCurrentIncrease() * f;
                        }
                    }
                }
            }
    ...
    public static Pair<MarketAPI, Float> getNearestSupCom(Vector2f locInHyper) {
        ...
        for (MarketAPI market : Misc.getFactionMarkets("player")) {
            if (market.hasIndustry(IndEvo_ids.SUPCOM)) {
                IndEvo_SupCom supCom = (IndEvo_SupCom) market.getIndustry(IndEvo_ids.SUPCOM);
                if (supCom.isFunctional() && supCom.getLastApplied() != null && supCom.getSpecialItem() != null) {
                    float dist = Misc.getDistanceLY(locInHyper, supCom.market.getLocationInHyperspace());
                    if (dist < minDist) {
                        minDist = dist;
                        nearest = market;
                    }
    ...
    public static float getDistanceIncomeMult(Vector2f locInHyper) {
        ...
        //return the current income multiplicator of the closest supCom with item times distance
        IndEvo_SupCom supCom = (IndEvo_SupCom) p.one.getIndustry(IndEvo_ids.SUPCOM);
        return supCom.getCurrentIncrease() * f;
    }
