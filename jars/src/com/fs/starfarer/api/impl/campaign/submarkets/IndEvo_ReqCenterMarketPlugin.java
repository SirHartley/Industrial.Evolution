...
import com.fs.starfarer.api.impl.campaign.econ.impl.IndEvo_ReqCenter;
...
public class IndEvo_ReqCenterMarketPlugin extends BaseSubmarketPlugin implements IndEvo_DynamicSubmarket {
    ...
    public void updateCargoPrePlayerInteraction() {
        ...
        if (okToUpdateShipsAndWeapons()) {
            ...
            if (getLocalAICoreId().equals(Commodities.GAMMA_CORE) || !IndEvo_ambassadorPersonManager.hasAmbassador(market)) {
            ...
            } else if (IndEvo_ambassadorPersonManager.hasAmbassador(market)) {

                //do a single faction selection
                FactionAPI faction = IndEvo_ambassadorPersonManager.getAmbassador(market).getFaction();
                int weapons = 7
    ...
    public static List<FactionAPI> getActiveFactionList(float minimumStanding, FactionAPI toFaction) {
        ...
        Set<String> blacklist = IndEvo_IndustryHelper.getCSVSetFromMemory(IndEvo_ids.ORDER_LIST);
    ...
    private String getLocalAICoreId() {
        ...
        if (market.hasIndustry(IndEvo_ids.REQCENTER) && market.getIndustry(IndEvo_ids.REQCENTER).getAICoreId() != null) {
            id = market.getIndustry(IndEvo_ids.REQCENTER).getAICoreId();
        }
    ...
    protected void createTooltipAfterDescription(TooltipMakerAPI tooltip, boolean expanded) {
        ...
        if (market.hasIndustry(IndEvo_ids.REQCENTER)) {
            ((IndEvo_ReqCenter) market.getIndustry(IndEvo_ids.REQCENTER)).publicAddAfterDescriptionSection(tooltip, Industry.IndustryTooltipMode.NORMAL);
        }
    }
