...
import com.fs.starfarer.api.impl.campaign.submarkets.IndEvo_ReqCenterMarketPlugin;
...
public class IndEvo_ReqCenter extends BaseIndustry {
    ...
    public void apply() {
        ...
        if (isFunctional()) {
            Global.getSector().addScript(new IndEvo_subMarketAddOrRemovePlugin(market, IndEvo_ids.REQMARKET, false));
        }
    }
    ...
    public void unapply() {
        ...
        Global.getSector().addScript(new IndEvo_subMarketAddOrRemovePlugin(market, IndEvo_ids.REQMARKET, true));
    }
    ...
    public void publicAddAfterDescriptionSection(TooltipMakerAPI tooltip, IndustryTooltipMode mode) {
        ...
        if (!isBuilding() && isFunctional() && mode.equals(IndustryTooltipMode.NORMAL)) {
            ...
            if (market.hasIndustry(IndEvo_ids.EMBASSY)
                    ...
                    && IndEvo_IndustryHelper.getAiCoreIdNotNull(this).equals(Commodities.GAMMA_CORE)) {

                FactionAPI alignedFaction = ((IndEvo_embassy) market.getIndustry(IndEvo_ids.EMBASSY)).alignedFaction;

                if (alignedFaction != null) {
                    float rel = marketFaction.getRelationship(alignedFaction.getId());
                    Pair<String, Color> standing = IndEvo_StringHelper.getRepIntTooltipPair(alignedFaction);
                    String weaponString;
                ...
                List<FactionAPI> activeFactionList = IndEvo_ReqCenterMarketPlugin.getActiveFactionList(MIN_REP_REQUIREMENT, marketFaction);
    ...
    private static String getAmountString(Float reputation, boolean longDesc) {
        ...
        return IndEvo_StringHelper.getString(IDENT, weaponString);
    }
...
actual effect handling is in IndEvo_ReqCenterMarketPlugin*/
