...
import com.fs.starfarer.api.impl.campaign.econ.impl.installableItemPlugins.IndEvo_BlueprintInstallableItemPlugin;
...
import static com.fs.starfarer.api.IndEvo_IndustryHelper.*;

public class IndEvo_EngHub extends IndEvo_SharedSubmarketUser implements IndEvo_newDayListener {

    public static final Logger log = Global.getLogger(IndEvo_EngHub.class);
    ...
    public void apply() {
        ...
        if (isFunctional() && market.isPlayerOwned()) {
            ...
            Global.getSector().addScript(new IndEvo_subMarketAddOrRemovePlugin(market, IndEvo_ids.ENGSTORAGE, false));
            refreshRequiredDays();
        }
    ...
    public void unapply() {
        ...
        if (blueprintItem != null) {
            IndEvo_BlueprintInstallableItemPlugin.BlueprintEffect effect = IndEvo_BlueprintInstallableItemPlugin.BLUEPRINT_EFFECTS.get(blueprintItem.getId());
            if (effect != null) {
                effect.unapply(this);
            }
        }
        ...
        Global.getSector().addScript(new IndEvo_subMarketAddOrRemovePlugin(market, IndEvo_ids.ENGSTORAGE, true));
    }
    ...
    public void onNewDay() {
        ...
        if (market.getSubmarket(Submarkets.SUBMARKET_STORAGE) == null
                || market.getSubmarket(IndEvo_ids.ENGSTORAGE) == null
                || !isFunctional())
            return;
        ...
        for (Map.Entry<String, Float> entry : researchProgressList.entrySet()) {
            ...
            if (entry.getValue() >= 1f) {
                ...
                if (dong) {
                    ...
                    //check Cargo
                    if (toRemove == null) {
                        if (market.hasSubmarket(IndEvo_ids.SHAREDSTORAGE)) {
    ...
    private boolean initDeconstruction() {
        CargoAPI engStorage = market.getSubmarket(IndEvo_ids.ENGSTORAGE).getCargo();
        ...
        for (FleetMemberAPI ship : engStorage.getMothballedShips().getMembersListCopy()) {
            ...
            ShipVariantAPI shipVar = IndEvo_IndustryHelper.stripShipToCargoAndReturnVariant(ship, market);
    ...
    public void setblueprintItem(SpecialItemData blueprintItem) {
        if (blueprintItem == null && this.blueprintItem != null) {
            IndEvo_BlueprintInstallableItemPlugin.BlueprintEffect effect = IndEvo_BlueprintInstallableItemPlugin.BLUEPRINT_EFFECTS.get(this.blueprintItem.getId());
            if (effect != null) {
                effect.unapply(this);
            }
        }
    ...
    public boolean wantsToUseSpecialItem(SpecialItemData data) {
        return blueprintItem == null &&
                data != null &&
                IndEvo_BlueprintInstallableItemPlugin.BLUEPRINT_EFFECTS.containsKey(data.getId());
    }
    ...
    public java.util.List<InstallableIndustryItemPlugin> getInstallableItems() {
        ...
        list.add(new IndEvo_BlueprintInstallableItemPlugin(this));
        return list;
    }
    ...
    public void initWithParams(java.util.List<String> params) {
        ...
        for (String str : params) {
            if (IndEvo_BlueprintInstallableItemPlugin.BLUEPRINT_EFFECTS.containsKey(str)) {
    ...
    public void addShipProgressOverview(TooltipMakerAPI tooltip, IndustryTooltipMode mode, boolean expanded) {
        if (!expanded || !mode.equals(IndustryTooltipMode.NORMAL)) return;
        refreshResearchProgressList();
...
//			label.setHighlightColors(h2, h1);
        ...
        if (mode.equals(IndustryTooltipMode.ADD_INDUSTRY) || !expanded) { //only display if either in add Industry or not expanded
