...
import static com.fs.starfarer.api.impl.campaign.econ.impl.IndEvo_AdAssem.DUAL_OUTPUT_REDUCTION_MULT;
import static com.fs.starfarer.api.impl.campaign.ids.IndEvo_Items.NO_ENTRY;

public class IndEvo_ConvertVPCSpecial extends BaseSalvageSpecial {
    //convert a VPC into ressources

    public static final Logger log = Global.getLogger(IndEvo_ConvertVPCSpecial.class);
    ...
    public static class ConvertVPCSpecialData implements SalvageSpecialInteraction.SalvageSpecialData {
        ...
        public SalvageSpecialInteraction.SalvageSpecialPlugin createSpecialPlugin() {
            return new IndEvo_ConvertVPCSpecial();
        }
    }

    private IndEvo_ConvertVPCSpecial.ConvertVPCSpecialData data;

    public IndEvo_ConvertVPCSpecial() {
    }

    @Override
    public void init(InteractionDialogAPI dialog, Object specialData) {
        super.init(dialog, specialData);
        data = (IndEvo_ConvertVPCSpecial.ConvertVPCSpecialData) specialData;
    ...
    public void optionSelected(String optionText, Object optionData) {
        if (SELECT.equals(optionData)) {
            ...
            for (String id : vpcList) {
                ...
                options.addOption(IndEvo_Items.getItemNameString(id), ident);
                options.addOptionConfirmation(ident,
                        "Are you sure? This will convert the VPC into " + IndEvo_Items.getVPCOutputString(id) + ".",
                        "Confirm",
                        "Return");
            }
            ...
            String prct = IndEvo_StringHelper.getAbsPercentString(data.qualityModifier, false);
            String id = optionData.toString().substring(VPC_IDENT.length());

            addText("The Forge accepts the chip without incident, and starts churning out " + IndEvo_Items.getVPCOutputString(id) + ".\n" +
    ...
    public Map<String, Integer> getDepositList(String id) {
        ...
        Pair<String, String> p = IndEvo_Items.getVPCCommodityIds(id);
    ...
    public int getDepositAmount(String commodityId, boolean dual) {
        ...
        try {
            amt = Global.getSettings().getJSONObject(IndEvo_ids.COMFORGE).getInt(commodityId);
            logIncrease = Global.getSettings().getJSONObject(IndEvo_ids.COMFORGE).getInt("logIncrease");
    ...
    private boolean playercargoHasVPC() {
        CargoAPI cargo = playerFleet.getCargo();
        Set<SpecialItemData> vpcIdSet = IndEvo_IndustryHelper.getVPCItemSet();
