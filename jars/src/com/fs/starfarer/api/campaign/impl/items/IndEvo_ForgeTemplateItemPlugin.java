...
import com.fs.starfarer.api.campaign.impl.items.specItemDataExt.IndEvo_ForgeTemplateData;
...
import static com.fs.starfarer.api.IndEvo_IndustryHelper.addOrIncrement;

public class IndEvo_ForgeTemplateItemPlugin extends BaseSpecialItemPlugin {
    ...
    public void createTooltip(TooltipMakerAPI tooltip, boolean expanded, CargoTransferHandlerAPI transferHandler, Object stackSource) {
        ...
        if (Global.getSector().getMemory().getBoolean("$" + IndEvo_ids.HULLFORGE)) {
    ...
    public static String pickShip(Set<String> tags, Random random) {
        ...
        Map<String, Float> dropMap = IndEvo_IndustryHelper.getMapFromMemory(DROP_MAP_KEY);

        restrictedShipSet.addAll(IndEvo_IndustryHelper.getVayraBossShips());
        restrictedShipSet.addAll(IndEvo_IndustryHelper.getPrismBossShips());

        Set<String> allowedShipsInternal = IndEvo_IndustryHelper.getCSVSetFromMemory(IndEvo_ids.PRINT_LIST);
        ...
        if (pick == null) {
            ...
            IndEvo_IndustryHelper.storeMapInMemory(dropMap, DROP_MAP_KEY);
    ...
    public static void addPrintDefectDMods(FleetMemberAPI member, int qualityLevel, Random random) {
        ...
        printDefectSet.add(IndEvo_ids.DEFECTS_LOW);
        printDefectSet.add(IndEvo_ids.DEFECTS_MED);
        printDefectSet.add(IndEvo_ids.DEFECTS_HIGH);

        boolean hasDefects = !Collections.disjoint(printDefectSet, hullmodSet);

        //always gets added
        if (!variant.hasHullMod(IndEvo_ids.PRINTING_INDICATOR)) variant.addPermaMod(IndEvo_ids.PRINTING_INDICATOR);
    ...
    public static void applyPrintDefects(FleetMemberAPI member, String aiCoreId) {
        ...
        if (!ship.hasHullMod(IndEvo_ids.PRINTING_INDICATOR)) ship.addPermaMod(IndEvo_ids.PRINTING_INDICATOR);
        ship.addPermaMod(id);
    ...
    public static int getForgeTemplateQualityLevel(SpecialItemData data) {
        if (data instanceof IndEvo_ForgeTemplateData) {
            IndEvo_ForgeTemplateData ftData = (IndEvo_ForgeTemplateData) data;
    ...
    public static SpecialItemData incrementForgeTemplateData(SpecialItemData data, int num) {
        ...
        if (getForgeTemplateCharges(data.getId()) > 1) {
            if (data instanceof IndEvo_ForgeTemplateData) {
                IndEvo_ForgeTemplateData ftData = (IndEvo_ForgeTemplateData) data;
                newData = new IndEvo_ForgeTemplateData(getIncrementedFTId(num, ftData.getId()), ftData.getData(), ftData.getVariant());
            } else {
                newData = new SpecialItemData(getIncrementedFTId(-1, data.getId()), data.getData());
            }

        } else {
            newData = new SpecialItemData(IndEvo_Items.BROKENFORGETEMPLATE, null);
        }
    ...
    public static String getIncrementedFTId(Integer increment, String initial) {
        ...
        if (charges > 0) {
            return IndEvo_Items.FORGETEMPLATE + "_" + (Math.min(charges + increment, 5));
        } else {
            return IndEvo_Items.BROKENFORGETEMPLATE;
        }
    }
    ...
    public static FleetMemberAPI createNakedFleetMemberFromForgeTemplate(SpecialItemData forgeTemplateData) {
        FleetMemberAPI ship;
        IndEvo_ForgeTemplateData data;

        if (forgeTemplateData instanceof IndEvo_ForgeTemplateData) {
            data = (IndEvo_ForgeTemplateData) forgeTemplateData;
    ...
    public static String getForgeTemplateHullID(SpecialItemData templateData) {
        if (templateData instanceof IndEvo_ForgeTemplateData) {
            IndEvo_ForgeTemplateData ftData = (IndEvo_ForgeTemplateData) templateData;
    ...
    public static IndEvo_ForgeTemplateData createForgeTemplateData(int charges, String hullId) {
        return new IndEvo_ForgeTemplateData(IndEvo_Items.FORGETEMPLATE + "_" + Math.min(charges, 5), hullId, Global.getSettings().createEmptyVariant(Misc.genUID(), Global.getSettings().getHullSpec(hullId)));
    }

    public static IndEvo_ForgeTemplateData createForgeTemplateData(int charges, String data, ShipVariantAPI variant) {
        return new IndEvo_ForgeTemplateData(IndEvo_Items.FORGETEMPLATE + "_" + Math.min(charges, 5), data, variant);
    }
}