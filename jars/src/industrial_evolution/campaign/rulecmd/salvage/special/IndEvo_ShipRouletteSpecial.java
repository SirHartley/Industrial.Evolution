...
import static industrial_evolution.IndEvo_IndustryHelper.stripShipToCargoAndReturnVariant;

public class IndEvo_ShipRouletteSpecial extends BaseSalvageSpecial {
    ...
    public static class ShipRouletteSpecialData implements SalvageSpecialInteraction.SalvageSpecialData {
        ...
        public SalvageSpecialInteraction.SalvageSpecialPlugin createSpecialPlugin() {
            return new IndEvo_ShipRouletteSpecial();
        }
    }

    private IndEvo_ShipRouletteSpecial.ShipRouletteSpecialData data;

    public IndEvo_ShipRouletteSpecial() {
    }

    @Override
    public void init(InteractionDialogAPI dialog, Object specialData) {
        super.init(dialog, specialData);
        data = (IndEvo_ShipRouletteSpecial.ShipRouletteSpecialData) specialData;
        if (data.hullIdDpMap.isEmpty()) initNothing();

        /*
        Global.getLogger(IndEvo_ShipRouletteSpecial.class).info("List RouletteSpecialShips:");
        for (Map.Entry<String, Float> e : data.hullIdDpMap.entrySet()){
            Global.getLogger(IndEvo_ShipRouletteSpecial.class).info(e.getKey() + " at " + e.getValue());
        }
*/
                ...
                cols, 88f, true, true, fleetMemberList, new FleetMemberPickerListener() {
                    ...
                    public void pickedFleetMembers(List<FleetMemberAPI> members) {
                        if (members != null && !members.isEmpty()) {
                            ...
                            Global.getLogger(IndEvo_ShipRouletteSpecial.class).info("Combined input DP " + totalDP);
                            ...
                            Global.getLogger(IndEvo_ShipRouletteSpecial.class).info("Player receives: " + bestID);
