...
import industrial_evolution.industries.derelicts.industry.IndEvo_HullForge;
...
public class IndEvo_PrintShipSpecial extends BaseSalvageSpecial {
    //prints a ship from a template

    public static Logger log = Global.getLogger(IndEvo_PrintShipSpecial.class);
    ...
    public static class PrintShipSpecialData implements SalvageSpecialInteraction.SalvageSpecialData {
        ...
        public SalvageSpecialInteraction.SalvageSpecialPlugin createSpecialPlugin() {
            return new IndEvo_PrintShipSpecial();
        }
    }

    private IndEvo_PrintShipSpecial.PrintShipSpecialData data;

    public IndEvo_PrintShipSpecial() {
    }

    @Override
    public void init(InteractionDialogAPI dialog, Object specialData) {
        super.init(dialog, specialData);
        data = (IndEvo_PrintShipSpecial.PrintShipSpecialData) specialData;
    ...
    private SpecialItemData reduceFTStack(FleetMemberAPI member) {
        ...
        SpecialItemData data = IndEvo_ForgeTemplateItemPlugin.incrementForgeTemplateData(stack.getSpecialDataIfSpecial(), -1);
    ...
    private void prepareAndAddMember(FleetMemberAPI member) {
        int quality = IndEvo_ForgeTemplateItemPlugin.getForgeTemplateQualityLevel(getForgeTemplateStackForMember(member).getSpecialDataIfSpecial())
                + data.qualityLevel;
        IndEvo_ForgeTemplateItemPlugin.addPrintDefectDMods(member, quality, random);
        IndEvo_HullForge.addBuiltInHullmods(1, member, random);
        //IndEvo_IndustryHelper.finalizeAndUpdateFleetMember(member);
                ...
                cols, 88f, true, false, fleetMemberList, new FleetMemberPickerListener() {
                    ...
                    public void pickedFleetMembers(List<FleetMemberAPI> members) {
                        if (members != null && !members.isEmpty()) {
                            ...
                            if (data.getId().equals(IndEvo_Items.BROKENFORGETEMPLATE))
    ...
    private CargoStackAPI getForgeTemplateStackForMember(FleetMemberAPI member) {
        for (CargoStackAPI stack : playerFleet.getCargo().getStacksCopy()) {
            ...
            if (isForgeTemplateStack(stack) && IndEvo_ForgeTemplateItemPlugin.getForgeTemplateHullID(stack.getSpecialDataIfSpecial()).equals(member.getHullId())) {
                return stack;
            }
        }
    ...
    private List<FleetMemberAPI> getForgeTemplateFleetMemberList() {
        ...
        for (CargoStackAPI stack : cargo.getStacksCopy()) {
            ...
            if (isForgeTemplateStack(stack))
                l.add(IndEvo_ForgeTemplateItemPlugin.createNakedFleetMemberFromForgeTemplate(stack.getSpecialDataIfSpecial()));
        }
