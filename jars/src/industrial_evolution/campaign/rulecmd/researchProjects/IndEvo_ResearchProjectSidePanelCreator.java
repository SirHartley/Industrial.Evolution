...
import industrial_evolution.splinterFleet.dialogue.customPanelPlugins.FramedCustomPanelPlugin;
...
public class IndEvo_ResearchProjectSidePanelCreator {
    public static final Logger log = Global.getLogger(IndEvo_ResearchProjectSidePanelCreator.class);
...
//check Nex_NGCStartFleetOptionsV2
    ...
    private void showCustomPanel() {
        ...
        for (Map.Entry<String, IndEvo_ResearchProject> projectEntry : IndEvo_ResearchProjectTemplateRepo.RESEARCH_PROJECTS.entrySet()) {

            final IndEvo_ResearchProject project = projectEntry.getValue();
            final String projId = project.getId();
            final IndEvo_ResearchProject.Progress progress = project.getProgress();
            ...
            InteractionDialogCustomPanelPlugin.ButtonEntry entry = new InteractionDialogCustomPanelPlugin.ButtonEntry(newLoadoutButton, buttonId) {
                ...
                public void onToggle() {
                    IndEvo_ResearchProjectDialoguePlugin.getCurrentDialoguePlugin().setProjectIdForInputs(projId);
                }
            };
            ...
            entry = new InteractionDialogCustomPanelPlugin.ButtonEntry(newLoadoutButton, buttonId) {
                @Override
                public void onToggle() {
                    IndEvo_ResearchProjectDonationCargoPicker.init(IndEvo_ResearchProjectDialoguePlugin.getCurrentDialoguePlugin(), projId);
                }
            };
            ...
            entry = new InteractionDialogCustomPanelPlugin.ButtonEntry(newLoadoutButton, buttonId) {
                @Override
                public void onToggle() {
                    IndEvo_ResearchProject project = IndEvo_ResearchProjectTemplateRepo.RESEARCH_PROJECTS.get(projId);
                    ...
                    IndEvo_ResearchProjectDialoguePlugin plugin = IndEvo_ResearchProjectDialoguePlugin.getCurrentDialoguePlugin();
            ...
            ButtonAPI checkbox = barHolder.addAreaCheckbox(IndEvo_StringHelper.getAbsPercentString(progressPercent, false), null, Misc.getDarkPlayerColor(), Misc.getDarkPlayerColor(), Misc.getTextColor(), Math.max(50f, holderWidth * progressPercent), BUTTON_HEIGHT, opad);
