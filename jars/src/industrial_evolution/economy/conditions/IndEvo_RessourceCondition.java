package industrial_evolution.economy.conditions;
...
import industrial_evolution.industries.IndEvo_SupCom;
...
import static industrial_evolution.campaign.ids.IndEvo_ids.COND_RESSOURCES;

public class IndEvo_RessourceCondition extends BaseMarketConditionPlugin {
    ...
    public void applySupComIncome() {
        for (MarketAPI m : Misc.getMarketsInLocation(market.getContainingLocation(), market.getFactionId())) {
            if (m.hasIndustry(IndEvo_ids.SUPCOM)) {
                return;
            }
        }

        Vector2f locInHyper = market.getLocationInHyperspace();
        Pair<MarketAPI, Float> p = IndEvo_SupCom.getNearestSupCom(locInHyper);
        float distMult = IndEvo_SupCom.getDistanceIncomeMult(locInHyper);
    ...
    private void applyPartsSupply(BaseIndustry ind, int supply) {
        ...
        if(ind.getSpecialItem() != null){
            ind.getSupply(IndEvo_Items.PARTS).getQuantity().unmodifyFlat("ind_sb");
        }
*/
        ind.supply(IndEvo_Items.PARTS, supply);

        ind.getSupply(IndEvo_Items.PARTS).getQuantity().unmodify(getModId());
        if (ind.getSpecialItem() != null) {
            if (ind.getSpecialItem().getId().equals(Items.CORRUPTED_NANOFORGE)) {
                ind.getSupply(IndEvo_Items.PARTS).getQuantity().modifyFlat(getModId(), -1, "Corrupted Nanoforge");
            }

            if (ind.getSpecialItem().getId().equals(Items.PRISTINE_NANOFORGE)) {
                ind.getSupply(IndEvo_Items.PARTS).getQuantity().modifyFlat(getModId(), -3, "Pristine Nanoforge");
            }
        }
    ...
    private void applyPartsDemands(BaseIndustry ind, int demand) {
        ...
        ind.demand(IndEvo_Items.PARTS, demand);

        ind.getDemand(IndEvo_Items.PARTS).getQuantity().unmodify(getModId());
        float qualityPenalty = MAX_QUALITY_PENALTY;

        if (ind.getSpecialItem() != null) {
            if (ind.getSpecialItem().getId().equals(Items.CORRUPTED_NANOFORGE)) {
                ind.getDemand(IndEvo_Items.PARTS).getQuantity().modifyFlat(getModId(), -1, "Corrupted Nanoforge");
                qualityPenalty = MAX_QUALITY_PENALTY_WITH_CORRUPTED;
            }

            if (ind.getSpecialItem().getId().equals(Items.PRISTINE_NANOFORGE)) {
                ind.getDemand(IndEvo_Items.PARTS).getQuantity().modifyFlat(getModId(), -2, "Pristine Nanoforge");
        ...
        Pair<String, Integer> deficit = ind.getMaxDeficit(IndEvo_Items.PARTS);
        if (deficit.two > 0) {
            float red = (deficit.two / (demand * 1f)) * qualityPenalty;
            ind.getMarket().getStats().getDynamic().getMod(Stats.PRODUCTION_QUALITY_MOD).modifyFlat(getModId(), red, Global.getSettings().getCommoditySpec(IndEvo_Items.PARTS).getName() + " demand not met");
        }
    ...
    public void unapplyAdditionalSupDem(BaseIndustry ind) {
        ind.supply(IndEvo_Items.PARTS, 0, "");
        ind.demand(IndEvo_Items.PARTS, 0, "");
    }
