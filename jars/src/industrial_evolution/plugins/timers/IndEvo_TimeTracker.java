package industrial_evolution.plugins.timers;
...
public class IndEvo_TimeTracker implements EveryFrameScript {
    ...
    private static void debugMessage(String Text) {
        ...
        if (DEBUG) {
            Global.getLogger(IndEvo_TimeTracker.class).info(Text);
        }
    }
    ...
    public static IndEvo_TimeTracker getTimeTrackerInstance() {
        for (EveryFrameScript s : Global.getSector().getScripts()) {
            if (s.getClass().equals(IndEvo_TimeTracker.class)) return (IndEvo_TimeTracker) s;
        }
    ...
    private void onNewDay() {
        List<IndEvo_newDayListener> list = Global.getSector().getListenerManager().getListeners(IndEvo_newDayListener.class);

        for (Iterator<IndEvo_newDayListener> i = list.iterator(); i.hasNext(); ) {
            IndEvo_newDayListener x = i.next();
