...
import industrial_evolution.industries.embassy.fleet.IndEvo_AmbassadorMurderConsequences;
...
import static industrial_evolution.plugins.ambassadorPlugins.IndEvo_ambassadorPersonManager.adjustRelationship;
import static industrial_evolution.plugins.ambassadorPlugins.IndEvo_ambassadorPersonManager.displayMessage;
...
public class IndEvo_ambassadorItemTrackerPlugin implements IndEvo_newDayListener {
        ...
        for (MarketAPI market : Global.getSector().getEconomy().getMarketsCopy()) {
            for (SubmarketAPI subMarket : market.getSubmarketsCopy()) {
                if (!subMarket.getSpecId().equals(Submarkets.SUBMARKET_STORAGE) && checkCargo(subMarket.getCargo(), amb, true)) {
                    ...
                    Global.getSector().addScript(new IndEvo_AmbassadorMurderConsequences(person));
                    setFailed();
                    return true;
                }
            }
    ...
    private void checkForWarConsequences() {
        if (IndEvo_ambassadorPersonManager.getListOfIncativeFactions().contains(faction)) {
        ...
        String memKey = IndEvo_AmbassadorIdHandling.getFactionMemoryKey(faction);
    ...
    public void setSuccessful() {
        ...
        if (m != null) {
            IndEvo_ambassadorPersonManager.addAmbassadorToMarket(person, m);
            setDone();
        } else
            Global.getLogger(IndEvo_ambassadorItemTrackerPlugin.class).error("IndEvo_ambassadorItemTrackerPlugin setSuccessful null value on getEmptyEmbassy");
    }

    public void setReturned() {
        MarketAPI market = IndEvo_ambassadorPersonManager.getOriginalMarket(person);
        ...
        IndEvo_ambassadorPersonManager.addAmbassadorToMarket(person, market);
        setDone();
    }
    ...
    public void setLost() {
        Global.getSector().getListenerManager().addListener(new IndEvo_lostAmbassadorRemovalChecker(person, specialItem));
        Global.getSector().getImportantPeople().removePerson(person);
        setDone();
    }
