...
public class IndEvo_edict_conscription extends IndEvo_baseEdict implements MarketImmigrationModifier, EconomyTickListener {
    ...
    public void apply(String id) {
        ...
        if (!IndEvo_TimeTracker.marketHasTimeTag(market, ident)) {
            market.addTag(ident + penaltyCounter);
        } else {
            penaltyCounter = IndEvo_TimeTracker.getTagNum(market, ident);
    ...
    public void reportEconomyTick(int iterIndex) {
        ...
        if (penaltyCounter > 0) {
            IndEvo_TimeTracker.incrementTagNum(market, ident, -1);
        }
    }
    ...
    public void removeWithoutPenalty() {
        IndEvo_TimeTracker.removeTimeTag(market, ident);
    ...
    public boolean isPresenceConditionMet(MarketAPI market) {
        return super.isPresenceConditionMet(market) && IndEvo_IndustryHelper.marketHasMilitaryIncludeRelays(market);
    }
