package industrial_evolution.industries.salvageyards.intel;
...
import industrial_evolution.campaign.rulecmd.salvageYardsRules.IndEvo_InitSYCustomProductionDiag;
...
import static industrial_evolution.IndEvo_StringHelper.getDayOrDays;
import static industrial_evolution.campaign.rulecmd.salvageYardsRules.IndEvo_InitSYCustomProductionDiag.DELIVERY_TIME;

public class IndEvo_YardsCustomProductionIntel extends BaseIntelPlugin implements IndEvo_newDayListener {
    ...
    public IndEvo_YardsCustomProductionIntel(MarketAPI deliverToMarket, IndEvo_InitSYCustomProductionDiag.YardsProductionData data) {
    ...
    public void addDescriptionForCurrentStage(TooltipMakerAPI info, float width, float height) {
        ...
        if (currentStage == IndEvo_YardsCustomProductionIntel.Stage.WAITING) {
        ...
        } else if (currentStage == IndEvo_YardsCustomProductionIntel.Stage.FAILED) {
    ...
    public boolean addNextStepText(TooltipMakerAPI info, Color tc, float pad) {
        ...
        if (currentStage == IndEvo_YardsCustomProductionIntel.Stage.WAITING) {
            ...
            return true;
        } else if (currentStage == IndEvo_YardsCustomProductionIntel.Stage.DELIVERED) {
    ...
    public void performStageActions() {
        ...
        if (market.hasCondition(Conditions.DECIVILIZED) || !market.hasIndustry(IndEvo_ids.SCRAPYARD)) {
    ...
    protected CargoAPI convertProdToCargo(IndEvo_InitSYCustomProductionDiag.YardsProductionData prod) {
        ...
        for (Map.Entry<String, Integer> e : prod.productionList.entrySet()) {
            ...
            for (int i = 0; i < count; i++) {
                String var = IndEvo_ForgeTemplateItemPlugin.getValidVariantIdForHullId(e.getKey());
    ...
    public void showCargoContents(TooltipMakerAPI info, float width, float height) {
        ...
        if (!tooltipCargo.getMothballedShips().getMembersListCopy().isEmpty()) {
            ...
            info.showShips(tooltipCargo.getMothballedShips().getMembersListCopy(), 20, true,
                    currentStage == IndEvo_YardsCustomProductionIntel.Stage.WAITING, opad);
        }
    }
}
