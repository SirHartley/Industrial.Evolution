...
import static industrial_evolution.industries.derelicts.industry.IndEvo_Ruins.INDUSTRY_ID_MEMORY_KEY;
...
public class IndEvo_RiftGen extends BaseIndustry implements IndEvo_newDayListener {
    ...
    private int getArtificialRiftAmount(StarSystemAPI system) {
        ...
        for (SectorEntityToken token : system.getJumpPoints()) {
            if (token.getId().contains(IndEvo_PlanetMovingScript.ARTIFICIAL_RIFT_ID)) count++;
        }
    ...
    private void movePlanet(StarSystemAPI target) {
        ...
        daysPassed = 0;
        IndEvo_PlanetMovingScript script = new IndEvo_PlanetMovingScript(market, target);
