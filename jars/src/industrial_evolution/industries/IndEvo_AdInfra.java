...
public class IndEvo_AdInfra extends BaseIndustry {

    public static Logger log = Global.getLogger(IndEvo_AdInfra.class);
    ...
    private Map<String, Integer> getIndustryExistOnMultipleMap() {
        ...
        Set<MarketAPI> targetMarkets = new HashSet<>(IndEvo_IndustryHelper.getMarketsInLocation(market.getStarSystem(), market.getFactionId()));
        ...
        if (getSpecialItem() != null) {
            for (MarketAPI m : Misc.getFactionMarkets(market.getFaction())) {
                ...
                if (dist <= IndEvo_SpecialItemEffectsRepo.RANGE_LY_TEN) {
                    targetMarkets.add(m);
                }
            }
        }
        ...
        for (MarketAPI m : targetMarkets) {
            ...
            for (String id : targetIds) {
                if (id.equals(IndEvo_ids.ADMANUF) && m.hasIndustry(IndEvo_ids.ADMANUF)) {
    ...
    private boolean coreIdIsAtWorst(String id) {
        String coreId = IndEvo_IndustryHelper.getAiCoreIdNotNull(this);
    ...
    private int getMaxSupportedInd() {
        if (getSpecialItem() != null) return IndEvo_SpecialItemEffectsRepo.LOG_CORE_MAX_BONUS;
    ...
    private Set<String> convertToLegalIDSet(List<Industry> indList) {
        ...
        Set<String> allowedIds = IndEvo_IndustryHelper.getCSVSetFromMemory(IndEvo_ids.BUREAU_LIST);
    ...
    public String getUnavailableReason() {
        ...
        if (!isOnlyInfraInSystem()) {
            return IndEvo_StringHelper.getString(getId(), "onlyOneInstanceAllowed");
        } else {
            return super.getUnavailableReason();
        }
    }
    ...
    protected void addRightAfterDescriptionSection(TooltipMakerAPI tooltip, IndustryTooltipMode mode) {
        ...
        if (!isBuilding()) {
            ...
            if (industryExistOnMultipleMap.isEmpty()) {
                String s = IndEvo_StringHelper.getString(getId(), "noDuplicates");
            ...
            if (isFunctional()) {
                if (coreIsSufficient()) {
                    String sufficient = IndEvo_StringHelper.getString(getId(), "sufficient");
                    tooltip.addPara(
                            IndEvo_StringHelper.getStringAndSubstituteToken(getId(), "aiCoreSufficient", "$sufficient", sufficient),
                            opad,
                            Misc.getPositiveHighlightColor(),
                            sufficient);

                } else if (!industryExistOnMultipleMap.isEmpty()) {
                    String insufficient = IndEvo_StringHelper.getString(getId(), "insufficient");
                    tooltip.addPara(
                            IndEvo_StringHelper.getStringAndSubstituteToken(getId(), "aiCoreInsufficient", "$insufficient", insufficient),
                            opad,
...
//Tooltip and descriptions:
    ...
    public void createTooltip(IndustryTooltipMode mode, TooltipMakerAPI tooltip, boolean expanded) {
        ...
        if (mode == IndustryTooltipMode.QUEUED) {
            ...
            tooltip.addPara("%s", opad, highlight, IndEvo_StringHelper.getString(getId(), "noAICore"));
        }
        ...
        if (!category) {
            ...
            if (hasSupply && isFunctional()) {
                tooltip.addSectionHeading(IndEvo_StringHelper.getString(getId(), "additionalOutputs"), color, dark, Alignment.MID, opad);
            ...
            if (hasDemand && isFunctional()) {
                tooltip.addSectionHeading(IndEvo_StringHelper.getString(getId(), "additionalDemands"), color, dark, Alignment.MID, opad);
                tooltip.beginIconGroup();
                tooltip.setIconSpacingMedium();
    ...
    public static Pair<MarketAPI, Float> getNearestBureauWithItem(Vector2f locInHyper) {
        ...
        for (MarketAPI market : Misc.getFactionMarkets("player")) {
            if (market.hasIndustry(IndEvo_ids.ADINFRA)) {
                Industry ind = market.getIndustry(IndEvo_ids.ADINFRA);
