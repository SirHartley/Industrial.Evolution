package industrial_evolution.industries.piratehaven.industry;
...
import industrial_evolution.industries.piratehaven.intel.IndEvo_RaidIntel;
...
import industrial_evolution.plugins.addOrRemovePlugins.IndEvo_industryAddOrRemovePlugin;
...
public class IndEvo_pirateHaven extends BaseIndustry implements EconomyTickListener, RaidIntel.RaidDelegate {
    ...
    public static final Logger log = Global.getLogger(IndEvo_pirateHaven.class);
    ...
    public void apply() {
        ...
        Global.getSector().addTransientScript(new IndEvo_industryAddOrRemovePlugin(market, IndEvo_ids.PIRATEHAVEN_SECONDARY, false));
    ...
    public void unapply() {
        ...
        Global.getSector().addTransientScript(new IndEvo_industryAddOrRemovePlugin(market, IndEvo_ids.PIRATEHAVEN_SECONDARY, true));
    }
    ...
    public boolean isAvailableToBuild() {
        return Global.getSettings().getBoolean("PirateHaven")
                ...
                && IndEvo_IndustryHelper.getAmountOfIndustryInSystem(getId(), market.getStarSystem(), market.getFaction()) < 2;
    }
    ...
    public String getUnavailableReason() {
        ...
        if (IndEvo_IndustryHelper.getAmountOfIndustryInSystem(getId(), market.getStarSystem(), market.getFaction()) > 1) {
    ...
    public void reportEconomyTick(int iterIndex) {
        ...
        if (isFunctional() && !market.hasCondition("IndEvo_pirate_subpop")) {
            market.addCondition("IndEvo_pirate_subpop");
        }
    ...
    public void notifyRaidEnded(RaidIntel raid, RaidIntel.RaidStageStatus status) {
        ...
        if (status == RaidIntel.RaidStageStatus.SUCCESS) {
            ...
            IndEvo_raidTimeout.addRaidedSystem(system, timeout, !market.isPlayerOwned());
            if (market.isPlayerOwned()) spoilsOfWar(system);

        } else {
            raidTimeoutMonths += 1;
            IndEvo_raidTimeout.addRaidedSystem(raid.getSystem(), 1f, !market.isPlayerOwned());
        ...
        if (withBP) {
            ...
            for (int i = 0; i < num && !picker.isEmpty(); i++) {
                ...
                if (id.startsWith(ship)) {
                    ...
                    if (IndEvo_EngHub.isTiandong(specId)) {
                        cargo.addSpecial(new SpecialItemData("tiandong_retrofit_bp", specId), 1);
                    } else if (IndEvo_EngHub.isRoider(specId)) {
    ...
    private StarSystemAPI getBestTargetSystem(FactionAPI faction) {
        ...
        for (StarSystemAPI system : getFactionStarSystemList(faction)) {
            ...
            if (IndEvo_raidTimeout.containsSystem(system, !market.isPlayerOwned())) {
    ...
    public void startRaid(StarSystemAPI target, float baseRaidFP) {
        ...
        for (MarketAPI curr : IndEvo_IndustryHelper.getMarketsInLocation(target)) {
        ...
        //make new raid
        RaidIntel raid = new IndEvo_RaidIntel(target, faction, this);
        ...
        currentIntel = raid;
        IndEvo_raidTimeout.addRaidedSystem(target, 3f, !market.isPlayerOwned());
    }
    ...
    public ArrayList<MarketAPI> getHostileMarketsInSystem(StarSystemAPI system) {
        ...
        for (MarketAPI market : IndEvo_IndustryHelper.getMarketsInLocation(system)) {
