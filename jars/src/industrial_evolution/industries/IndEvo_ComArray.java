...
import industrial_evolution.items.installable.IndEvo_SpecialItemEffectsRepo;
import com.fs.starfarer.api.impl.campaign.fleets.RouteManager;
import com.fs.starfarer.api.impl.campaign.ids.*;
import industrial_evolution.campaign.ids.IndEvo_ids;
import industrial_evolution.plugins.addOrRemovePlugins.IndEvo_entityRemovalScript;
...
import static industrial_evolution.items.installable.IndEvo_SpecialItemEffectsRepo.RANGE_LY_TEN;


public class IndEvo_ComArray extends MilitaryBase implements IndEvo_newDayListener {
    ...
    public void apply() {
        ...
        if (getId().equals(IndEvo_ids.INTARRAY)) {
            if (getSpecialItem() == null) createCommRelayStation(IndEvo_ids.INTARRAY_ENTITY);
            else createCommRelayStation(IndEvo_ids.PRISTINE_INTARRAY_ENTITY);
        }

        if (!marketHasMilitary()) {
            getUpkeep().modifyMult("ind_patHQ", PATROL_HQ_UPKEEP_MULT, IndEvo_StringHelper.getString(STRING_IDENT, "operateAsHQ"));
            patrolHqApply();
        } else {
    ...
    private void createCommRelayStation(String id) {
        ...
        for (CustomCampaignEntityAPI entity : market.getContainingLocation().getCustomEntitiesWithTag(IndEvo_ids.INTARRAY_ENTITY_TAG)) {
    ...
    private void removeCommRelayStation() {
        ...
        if (relay != null) {
            ...
            for (EveryFrameScript s : Global.getSector().getTransientScripts()) {
                if (s.getClass().getName().equals("IndEvo_entityRemovalScript")) {
                    return;
                }
            }

            Global.getSector().addTransientScript(new IndEvo_entityRemovalScript(relay, market, getId(), addedOrRemovedItem));
    ...
    private float calculateNetworkwideHighestFS() {
        boolean systemHasIA = IndEvo_IndustryHelper.systemHasIndustryExcludeNotFunctional(IndEvo_ids.INTARRAY, market.getStarSystem(), market.getFaction());
        ...
        if (systemHasIA) {
            ...
            for (StarSystemAPI system : playerSystemList) {
                if (IndEvo_IndustryHelper.systemHasIndustryExcludeNotFunctional(IndEvo_ids.INTARRAY, system, faction)) {
    ...
    private Pair<String, Float> getBestPairInSystem(StarSystemAPI system, FactionAPI faction) {
        ...
        List<MarketAPI> PlayerMarketsInSystem = IndEvo_IndustryHelper.getMarketsInLocation(system, faction.getId());
        for (MarketAPI playerMarket : PlayerMarketsInSystem) {
            if (playerMarket.hasIndustry(IndEvo_ids.COMARRAY) || playerMarket.hasIndustry(IndEvo_ids.INTARRAY)) {
    ...
    private boolean transmitterPresent() {
        return Global.getSector().getMemoryWithoutUpdate().getBoolean(IndEvo_SpecialItemEffectsRepo.TRANSMITTER_UNLOCK_KEY);
    }

    private boolean systemHasMB() {
        return IndEvo_IndustryHelper.systemHasIndustry(Industries.MILITARYBASE, market.getStarSystem(), market.getFaction());
    }

    private boolean systemHasHC() {
        return IndEvo_IndustryHelper.systemHasIndustry(Industries.HIGHCOMMAND, market.getStarSystem(), market.getFaction());
    }

    private boolean marketHasMilitary() {
        return IndEvo_IndustryHelper.marketHasMilitary(market, false);
    }
    ...
    public String getUnavailableReason() {
        if (!isFunctionAllowed()) {
            return IndEvo_StringHelper.getString(STRING_IDENT, "unavailableReason");
        }
    ...
    public float getBuildTime() {
        boolean isIntArray = getId().equals(IndEvo_ids.INTARRAY);
    ...
    protected void addRightAfterDescriptionSection(TooltipMakerAPI tooltip, IndustryTooltipMode mode) {
        boolean ia = getId().equals(IndEvo_ids.INTARRAY);
        ...
        if (!isBuilding() && isFunctional()) {
            ...
            if (market.isPlayerOwned() && currTooltipMode == IndustryTooltipMode.NORMAL) {
                if (isFunctional()) {
                    ...
                    toReplace.put("$fleetSize", "+" + IndEvo_StringHelper.getAbsPercentString(value, false));
                    toReplace.put("$market", bestMarket.getName());
                    toReplace.put("$system", bestMarket.getStarSystem().getName());

                    String currentBonusTooltip = IndEvo_StringHelper.getStringAndSubstituteTokens(STRING_IDENT, ia ? "currentBonusInt" : "currentBonus", toReplace);
                    String hcName = Global.getSettings().getIndustrySpec(Industries.HIGHCOMMAND).getName();

                    if (market.getId().equals(bestMarketId))
                        tooltip.addPara(IndEvo_StringHelper.getString(STRING_IDENT, "isBest"), opad, highlight, IndEvo_StringHelper.getString(STRING_IDENT, "isBestHighlights"));
                    else
                    ...
                    if (!systemHasHC())
                        tooltip.addPara(IndEvo_StringHelper.getString(STRING_IDENT, "canImprove"), 2f, highlight, hcName);

                    tooltip.addPara(IndEvo_StringHelper.getString(STRING_IDENT, "aiCoreNotice"), opad, highlight, new String[]{IndEvo_StringHelper.getString("IndEvo_AICores", "aiCores"), hcName});
                }
            }

            if (ia) {
                tooltip.addPara(IndEvo_StringHelper.getString(STRING_IDENT, "commRelayNotice"), opad, Misc.getPositiveHighlightColor(), Global.getSettings().getCustomEntitySpec(Entities.COMM_RELAY_MAKESHIFT).getNameInText());
            }
        }
    }
...
//AI core Handling
    ...
    private String getBestHighCommandAICoreId() {
        List<MarketAPI> marketsInLocation = IndEvo_IndustryHelper.getMarketsInLocation(market.getStarSystem(), market.getFactionId());
    ...
    public void addInstalledItemsSection(IndustryTooltipMode mode, TooltipMakerAPI tooltip, boolean expanded) {
        ...
        LabelAPI heading = tooltip.addSectionHeading(Misc.ucFirst(IndEvo_StringHelper.ucFirstIgnore$(IndEvo_StringHelper.getString("IndEvo_items", "items"))), color, dark, Alignment.MID, opad);
        ...
        if (aiCoreId != null) {
            tooltip.addPara("%s",
                    ...
                    IndEvo_StringHelper.getString(STRING_IDENT, "aiCoreNoEffect"));
        ...
        if (!addedSomething) {
            heading.setText(IndEvo_StringHelper.getString("IndEvo_items", "noItems"));
        }
    }
    ...
    protected void addAlphaCoreDescription(TooltipMakerAPI tooltip, AICoreDescriptionMode mode) {
        ...
        String effect = IndEvo_StringHelper.getString(STRING_IDENT, "aCoreEffect");
        String highlightString = IndEvo_StringHelper.getAbsPercentString(ALPHA_CORE_BONUS_FS_MULT, true);
        ...
        if (mode == AICoreDescriptionMode.INDUSTRY_TOOLTIP) {
            ...
            tooltip.addPara("%s", opad, Misc.getNegativeHighlightColor(), IndEvo_StringHelper.getString(STRING_IDENT, "aiCoreNoEffect"));
        }
    }
    ...
    protected void addBetaCoreDescription(TooltipMakerAPI tooltip, AICoreDescriptionMode mode) {
        ...
        String effect = IndEvo_StringHelper.getString(STRING_IDENT, "bCoreEffect");
    ...
    protected void addGammaCoreDescription(TooltipMakerAPI tooltip, AICoreDescriptionMode mode) {
        ...
        String effect = IndEvo_StringHelper.getString(STRING_IDENT, "gCoreEffect");
    ...
    protected void applyAICoreToIncomeAndUpkeep() {
        String name = IndEvo_StringHelper.getString("IndEvo_AICores", "bCoreStatModAssigned");
    ...
    public void createTooltip(IndustryTooltipMode mode, TooltipMakerAPI tooltip, boolean expanded) {
        ...
        String type = "";
        if (isIndustry()) type = IndEvo_StringHelper.getString(cat, "t1");
        if (isStructure()) type = IndEvo_StringHelper.getString(cat, "t2");
        ...
        if (isIndustry() && (mode == IndustryTooltipMode.ADD_INDUSTRY ||
            ...
            if (num > max) {// || (num >= max && mode == IndustryTooltipMode.ADD_INDUSTRY)) {
                ...
                tooltip.addPara(IndEvo_StringHelper.getString(cat, "t3"), bad, opad);
            }
        }
        ...
        if (isDisrupted()) {
            ...
            tooltip.addPara(IndEvo_StringHelper.getStringAndSubstituteToken(cat, "t4", "$days", IndEvo_StringHelper.getDayOrDays(left)),
        ...
        if (DebugFlags.COLONY_DEBUG || market.isPlayerOwned()) {
            if (mode == IndustryTooltipMode.NORMAL) {
                if (getSpec().getUpgrade() != null && !isBuilding()) {
                    tooltip.addPara(IndEvo_StringHelper.getString(cat, "t5"), Misc.getPositiveHighlightColor(), opad);
                } else {
                    tooltip.addPara(IndEvo_StringHelper.getString(cat, "t6"), Misc.getPositiveHighlightColor(), opad);
                }
        ...
        if (mode == IndustryTooltipMode.QUEUED) {
            tooltip.addPara(IndEvo_StringHelper.getString(cat, "t7"), Misc.getPositiveHighlightColor(), opad);
            tooltip.addPara(IndEvo_StringHelper.getString(cat, "t8"), opad);

            int left = (int) (getSpec().getBuildTime());
            if (left < 1) left = 1;
            tooltip.addPara(IndEvo_StringHelper.getStringAndSubstituteToken(cat, "t9", "$days", IndEvo_StringHelper.getDayOrDays(left)), opad, highlight, "" + left);

            //return;
        } else if (!isFunctional() && mode == IndustryTooltipMode.NORMAL && isBuilding()) {
            tooltip.addPara(IndEvo_StringHelper.getString(cat, "t10"), opad);

            int left = (int) (buildTime - buildProgress);
            if (left < 1) left = 1;
            tooltip.addPara(IndEvo_StringHelper.getStringAndSubstituteToken(cat, "t11", "$days", IndEvo_StringHelper.getDayOrDays(left)), opad, highlight, "" + left);
        } else if (!isFunctional() && !isFunctionAllowed() && mode == IndustryTooltipMode.NORMAL) {
            tooltip.addPara("%s", opad, bad, IndEvo_StringHelper.getString(STRING_IDENT, "notFunctional"));
        }
        ...
        if (!category) {
            ...
            if (mode == IndustryTooltipMode.UPGRADE || mode == IndustryTooltipMode.ADD_INDUSTRY) {
                ...
                LabelAPI label = null;
                if (mode == IndustryTooltipMode.UPGRADE) {
                    label = tooltip.addPara(IndEvo_StringHelper.getStringAndSubstituteToken(cat, "t12", "$days", IndEvo_StringHelper.getDayOrDays(days)), opad,
                            highlight, costStr, "" + days, creditsStr);
                } else {
                    label = tooltip.addPara(IndEvo_StringHelper.getStringAndSubstituteToken(cat, "t13", "$days", IndEvo_StringHelper.getDayOrDays(days)), opad,
                            highlight, costStr, "" + days, creditsStr);
                }
                ...
                tooltip.addPara(IndEvo_StringHelper.getString(cat, "t14"), opad, highlight, refundStr);
            }
            ...
            if (!getIncome().isUnmodified()) {
                ...
                tooltip.addPara(IndEvo_StringHelper.getString(cat, "t15"), opad, highlight, Misc.getDGSCredits(income));
            ...
            if (!getUpkeep().isUnmodified()) {
                ...
                tooltip.addPara(IndEvo_StringHelper.getString(cat, "t16"), opad, highlight, Misc.getDGSCredits(upkeep));
            ...
            if (hasSupply) {
                tooltip.addSectionHeading(IndEvo_StringHelper.getString(cat, "t17"), color, dark, Alignment.MID, opad);
                ...
                for (MutableCommodityQuantity curr : supply.values()) {
                    ...
                    for (MutableStat.StatMod mod : curr.getQuantity().getFlatMods().values()) {
                        if (mod.value > 0) {
                            plus += (int) mod.value;
                        } else if (mod.desc != null && mod.desc.contains(IndEvo_StringHelper.getString("shortage"))) {
            ...
            if (hasDemand || hasPostDemandSection(hasDemand, mode)) {
                tooltip.addSectionHeading(IndEvo_StringHelper.getString(cat, "t18"), color, dark, Alignment.MID, opad);
            }
            if (hasDemand) {
                tooltip.beginIconGroup();
                tooltip.setIconSpacingMedium();
            ...
            tooltip.addPara(IndEvo_StringHelper.getString(cat, "t19"), gray, opad);
        }
