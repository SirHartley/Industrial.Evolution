...
public class IndEvo_ArtilleryShotScript implements EveryFrameScript {
    ...
    public IndEvo_ArtilleryShotScript (SectorEntityToken origin, SectorEntityToken target, int amt){
        this.origin = origin;
        this.target = target;
        this.amt = amt;
    }
    ...
    public void advance(float amount) {
        ...
        for (Vector2f t : targetList){
            IndEvo_ArtilleryProjectileEntityPlugin.spawn(origin.getContainingLocation(), origin, t, (float) (AVERAGE_PROJ_IMPACT_TIME + 3f * random()));
        }
