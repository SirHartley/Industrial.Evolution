...
public class IndEvo_ambassadorPersonManager implements EconomyTickListener, NewDayListener {
    ...
    private void getWarConsequences() {
        ...
        for (MarketAPI market : getPlayerMarketsWithEmbassy()) {
            ...
            String memKey = IndEvo_AmbassadorItemHelper.getFactionMemoryKey(getAmbassador(market).getFaction());
    ...
    public static void createAmbassadorsOnMarkets(MarketAPI market) {
        ...
        String memKey = IndEvo_AmbassadorItemHelper.getFactionMemoryKey(market.getFaction());
        if (Global.getSector().getMemory().contains(memKey)
                && !Global.getSector().getMemory().getBoolean(memKey)) {
            return;
        }

        if (IndEvo_AmbassadorItemHelper.getAllowedFactionID().contains(market.getFaction().getId())
                && market.getSize() >= 5
    ...
    public static PersonAPI getAmbassador(MarketAPI market) {
        ...
        for (CommDirectoryEntryAPI dir : market.getCommDirectory().getEntriesCopy()) {
            ...
            if (person.getPostId().equals(IndEvo_AmbassadorItemHelper.POST_AMBASSADOR))
                return person;
        }
        return null;
    }
    ...
    public static PersonAPI createAmbassador(MarketAPI market, boolean noDuplicate, String factionIdOverride) {
        ...
        ImportantPeopleAPI ip = Global.getSector().getImportantPeople();
        String rankId = IndEvo_AmbassadorItemHelper.RANK_AMBASSADOR;

        PersonAPI person = market.getFaction().createRandomPerson();
        person.setRankId(rankId);
        person.setPostId(IndEvo_AmbassadorItemHelper.POST_AMBASSADOR);
