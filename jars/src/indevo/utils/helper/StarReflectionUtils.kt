...
internal object StarReflectionUtils {
    ...
    private val getConstructorParametersHandle =
        StarReflectionUtils.constructorClass.getHandle("getParameterTypes", arrayOf<Class<*>>().javaClass)
    private val setConstructorAccessibleHandle =
        StarReflectionUtils.constructorClass.getHandle("setAccessible", Void.TYPE, Boolean::class.javaPrimitiveType)
    private val invokeConstructorHandle =
        StarReflectionUtils.constructorClass.getHandle("newInstance", Any::class.java, arrayOf<Any>()::class.java)
    ...
    private val reflectedMethodsCache = ConcurrentHashMap<StarReflectionUtils.ReflectedMethodCacheKey, List<ReflectedMethod>>()
    ...
    private data class ReflectedMethodCacheKey(
        ...
        override fun equals(other: Any?): Boolean {
            ...
            other as StarReflectionUtils.ReflectedMethodCacheKey
    ...
    private val reflectedFieldsCache = ConcurrentHashMap<StarReflectionUtils.ReflectedFieldCacheKey, List<ReflectedField>>()
    ...
    private data class ReflectedFieldWithMethodCacheKey(
        ...
        override fun equals(other: Any?): Boolean {
            ...
            other as StarReflectionUtils.ReflectedFieldWithMethodCacheKey
    ...
    private val reflectedFieldsWithMethodCache = ConcurrentHashMap<StarReflectionUtils.ReflectedFieldWithMethodCacheKey, List<ReflectedField>>()
    ...
    private data class ReflectedConstructorsCacheKey(
        ...
        override fun equals(other: Any?): Boolean {
            ...
            other as StarReflectionUtils.ReflectedConstructorsCacheKey
    ...
    private val ReflectedConstructorsCache = ConcurrentHashMap<StarReflectionUtils.ReflectedConstructorsCacheKey, List<StarReflectionUtils.ReflectedConstructor>>()
    ...
    fun Class<*>.getFieldsMatching(
        ...
        val cacheKey = StarReflectionUtils.ReflectedFieldCacheKey(
            targetClass = this,
            fieldName = name,
            fieldType = type,
        ...
        return StarReflectionUtils.reflectedFieldsCache.getOrPut(cacheKey) {
            (if (searchSuperclass) StarReflectionUtils.getAllFields(this) else this.declaredFields.toSet()).filter { field ->
                // 1. Check Name
                // Use handle to get name and compare if a specific name is requested.
                if (name != null && name != StarReflectionUtils.getFieldNameHandle.invoke(field)) return@filter false

                // --- Field Checks ---
                // Fetch actual field types using the handle *only if needed*.
                if (type != null || fieldAccepts != null || fieldAssignableTo != null) {
                    val fieldType = StarReflectionUtils.getFieldTypeHandle.invoke(field) as Class<*>
                    // 2. Check the exact type
                    if (type != null && type != fieldType) return@filter false

                    // 3. Check if we can (set with) / (get a) provided type
                    if (fieldAccepts != null && !StarReflectionUtils.isParameterCompatible(fieldType, fieldAccepts)) return@filter false
    ...
    fun Class<*>.getFieldsWithMethodsMatching(
        ...
        val cacheKey = StarReflectionUtils.ReflectedFieldWithMethodCacheKey(
            targetClass = this,
        ...
        return StarReflectionUtils.reflectedFieldsWithMethodCache.getOrPut(cacheKey){
            (if (searchSuperclass) StarReflectionUtils.getAllFields(this) else this.declaredFields.toSet()).filter { fieldInstance ->
                val fieldType = StarReflectionUtils.getFieldTypeHandle.invoke(fieldInstance) as Class<*>?
    ...
    private val wrapperToPrimitive = StarReflectionUtils.primitiveToWrapper.entries.associate { (k, v) -> v to k }
    ...
    private fun isParameterCompatible(methodParamType: Class<*>, callerArgType: Class<*>?): Boolean {
        ...
        return when {
            ...
            callerArgType.isPrimitive && methodParamType.isPrimitive ->
                StarReflectionUtils.primitiveWidensFrom[methodParamType]?.contains(callerArgType) ?: false
            ...
            !methodParamType.isPrimitive && callerArgType.isPrimitive -> {
                val boxedCallerType = StarReflectionUtils.primitiveToWrapper[callerArgType]
            ...
            !callerArgType.isPrimitive && methodParamType.isPrimitive -> {
                val unboxedCallerType = StarReflectionUtils.wrapperToPrimitive[callerArgType]
                if (unboxedCallerType != null)
                    unboxedCallerType == methodParamType || (StarReflectionUtils.primitiveWidensFrom[methodParamType]?.contains(unboxedCallerType) ?: false)
                else false
            }
    ...
    fun Class<*>.getMethodsMatching(
        ...
        val cacheKey = StarReflectionUtils.ReflectedMethodCacheKey(
            targetClass = this,
            fieldName = name,
        ...
        return StarReflectionUtils.reflectedMethodsCache.getOrPut(cacheKey) {
            val accessibleMethods: Set<Any> = (this.declaredMethods + this.methods).toSet()

            accessibleMethods.filter { method ->
                // 1. Check Name
                // Use handle to get name and compare if a specific name is requested.
                if (name != null && name != StarReflectionUtils.getMethodNameHandle.invoke(method)) return@filter false

                // 2. Check Return Type
                if (returnType != null) {
                    val actualReturnType = StarReflectionUtils.getMethodReturnHandle.invoke(method) as Class<*>
                ...
                if (numOfParams != null || parameterTypes != null){
                    ...
                    val actualParamTypes = StarReflectionUtils.getMethodParametersHandle.invoke(method) as Array<Class<*>>
                    ...
                    if (parameterTypes != null) {
                        ...
                        parameterTypes.forEachIndexed { index, callerArgType ->
                            if (!StarReflectionUtils.isParameterCompatible(actualParamTypes[index], callerArgType)) return@filter false
                        }
                    }
                }
    ...
    fun getConstructorsMatching(
        ...
        numOfParams: Int? = null,
        parameterTypes: Array<Class<*>?>? = null
    ): List<StarReflectionUtils.ReflectedConstructor> { return clazz.getConstructorsMatching(numOfParams, parameterTypes)}
    ...
    fun Class<*>.getConstructorsMatching(
        numOfParams: Int? = null,
        parameterTypes: Array<Class<*>?>? = null
    ): List<StarReflectionUtils.ReflectedConstructor> {
        val cacheKey = StarReflectionUtils.ReflectedConstructorsCacheKey(
            targetClass = this,
            numOfParams = numOfParams,
            parameterTypes = parameterTypes
        )

        return StarReflectionUtils.ReflectedConstructorsCache.getOrPut(cacheKey){
            this.declaredConstructors.filter { constructor ->
                // Get actual parameters
                @Suppress("UNCHECKED_CAST")
                val actualParamTypes = StarReflectionUtils.getConstructorParametersHandle.invoke(constructor) as Array<Class<*>>
                ...
                return@filter true
            }.map { StarReflectionUtils.ReflectedConstructor(it) }
        }
    ...
    class ReflectedField(private val field: Any) {
        val type: Class<*> = StarReflectionUtils.getFieldTypeHandle.invoke(field) as Class<*>
        val name: String = StarReflectionUtils.getFieldNameHandle.invoke(field) as String
        /**
        ...
        fun get(instance: Any?): Any? {
            StarReflectionUtils.setFieldAccessibleHandle.invoke(field, true)
            return StarReflectionUtils.getFieldHandle.invoke(field, instance)
        }
        ...
        fun set(instance: Any?, value: Any?) {
            StarReflectionUtils.setFieldAccessibleHandle.invoke(field, true)
            StarReflectionUtils.setFieldHandle.invoke(field, instance, value)
        }
    }
    ...
    class ReflectedMethod(private val method: Any) {
        @Suppress("UNCHECKED_CAST")
        val parameterTypes: Array<Class<*>> = StarReflectionUtils.getMethodParametersHandle.invoke(method) as Array<Class<*>>
        val returnType: Class<*>? = StarReflectionUtils.getMethodReturnHandle.invoke(method) as Class<*>?
        val name: String = StarReflectionUtils.getMethodNameHandle.invoke(method) as String
        /**
        ...
        fun invoke(instance: Any?, vararg arguments: Any?): Any? {
            StarReflectionUtils.setMethodAccessibleHandle.invoke(method, true)
            return StarReflectionUtils.invokeMethodHandle.invoke(method, instance, arguments)
        }
    }
    ...
    class ReflectedConstructor(private val constructor: Any) {
        @Suppress("UNCHECKED_CAST")
        val parameterTypes: Array<Class<*>> = StarReflectionUtils.getConstructorParametersHandle.invoke(constructor) as Array<Class<*>>
        /**
        ...
        fun newInstance(vararg arguments: Any?): Any {
            StarReflectionUtils.setConstructorAccessibleHandle.invoke(constructor, true)
            return StarReflectionUtils.invokeConstructorHandle.invoke(constructor, arguments)
        }
    }
}