...
public class IceSwarmSpawner extends BaseArcingSwarmSpawner{
    ...
    public static final int BASE_WIDTH_PER_ASTEROID_PER_SECOND = 700;
    ...
    void advanceSpawner(float amount) {
        ...
        if (timePassed / runtime >= MIN_TREASURE_SPAWN_FRACT && timePassed / runtime <= MAX_TREASURE_SPAWN_FRACT){
            ...
            if (treasureSpawned < treasureAmt){
                ...
                if (treasureInterval.intervalElapsed()){
                    ...
                    ModPlugin.log("spawning roid size " + data.size);
        ...
        if (roll < chance && random.nextBoolean()) {
            MeteorEntity.MeteorData data = new MeteorEntity.MeteorData(size * 2, new ArcingMovementModule( arc.getModifiedRadiusArc(arc.radius + distFromLine), speed));
            ModPlugin.log("spawning roid size " + size * 2);
            MeteorFactory.spawn(system, data, MeteorSwarmManager.MeteroidShowerType.ICEROID);
            return;
        }
