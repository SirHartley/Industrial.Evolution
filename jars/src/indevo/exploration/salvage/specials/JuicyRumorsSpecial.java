...
public class JuicyRumorsSpecial extends BaseSalvageSpecial {
    ...
    public static class JuicyRumorsSpecialData implements SalvageSpecialInteraction.SalvageSpecialData {
        ...
        public SalvageSpecialInteraction.SalvageSpecialPlugin createSpecialPlugin() {
            return new JuicyRumorsSpecial();
        }
    }

    private JuicyRumorsSpecial.JuicyRumorsSpecialData data;

    public JuicyRumorsSpecial() {
    }
    ...
    public void init(InteractionDialogAPI dialog, Object specialData) {
        ...
        data = (JuicyRumorsSpecial.JuicyRumorsSpecialData) specialData;
    ...
    public void optionSelected(String optionText, Object optionData) {
        if (optionData.equals(FACTION_1)) {
            ...
            h.setText(StringHelper.getFloatToIntStrx100(data.repGainAmt), faction_1.getDisplayNameLongWithArticle(), repInt.one);
            ...
            //faction 1 rep loss
            Pair<String, Color> repInt = StringHelper.getRepIntTooltipPair(faction_1);
            text.addPara("Lost " + StringHelper.getFloatToIntStrx100(repReduce) + " reputation with " + faction_1.getDisplayNameWithArticle() + " - Now at " + repInt.one);

            Highlights h = new Highlights();
            h.setText(StringHelper.getFloatToIntStrx100(repReduce), faction_1.getDisplayNameLongWithArticle(), repInt.one);
            h.setColors(Misc.getNegativeHighlightColor(), faction_1.getColor(), repInt.two);
            text.setHighlightsInLastPara(h);

            //faction 2 rep gain
            Pair<String, Color> repInt2 = StringHelper.getRepIntTooltipPair(faction_2);
            text.addPara("Gained " + StringHelper.getFloatToIntStrx100(repIncrease) + " reputation with " + faction_2.getDisplayNameWithArticle() + " - Now at " + repInt2.one);

            Highlights h2 = new Highlights();
            h2.setText(StringHelper.getFloatToIntStrx100(repIncrease), faction_2.getDisplayNameLongWithArticle(), repInt2.one);
